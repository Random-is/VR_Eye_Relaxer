...
namespace Gvr.Internal {
    ...
    public class InstantPreview : MonoBehaviour {
        ...
        void Start() {
            ...
            if (InstallApkOnRun) {
                ...
                if (InstantPreviewApk == null) {
                    Debug.LogError(
                        "Trying to install Instant Preview apk but reference to InstantPreview.apk is broken.");
                    return;
                }
                        ...
                        () => {
                            ...
                            if (!string.IsNullOrEmpty(output) && string.IsNullOrEmpty(errors)) {
                                ...
                                if (unityAPKVersionNameRegex.Groups.Count > 1) {
                                    ...
                                    Debug.Log(string.Format("Failed to extract version from: {0}",
                                        unityAPKVersionInfoDump));
                                }
                            }
                            else {
                                Debug.Log(string.Format("Failed to run: {0} dump badging {1}",
                                    InstantPreviewHelper.aaptPath, apkPath));
                            }
        ...
        void EnsureCamera(Camera camera) {
            ...
            EnsureEyeCamera(camera, ":Instant Preview Left", new Rect(0.0f, 0.0f, 0.5f, 1.0f),
                ref eyeCamera.leftEyeCamera);
            EnsureEyeCamera(camera, ":Instant Preview Right", new Rect(0.5f, 0.0f, 0.5f, 1.0f),
                ref eyeCamera.rightEyeCamera);
        }
