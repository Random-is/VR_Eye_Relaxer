...
namespace Gvr.Internal {
    public class AndroidNativeKeyboardProvider : IKeyboardProvider {
...
#if UNITY_ANDROID
        [DllImport(dllName)]
        private static extern IntPtr gvr_keyboard_initialize(AndroidJavaObject app_context,
            AndroidJavaObject class_loader);
#endif
        [DllImport(dllName)]
        ...
        [DllImport(dllName)]
        private static extern void gvr_keyboard_update_button_state(IntPtr keyboard_context, int buttonIndex,
            bool pressed);
        ...
        [DllImport(dllName)]
        private static extern void gvr_keyboard_update_controller_touch(IntPtr keyboard_context, bool touched,
            IntPtr vector2Pos);
        ...
        [DllImport(dllName)]
        private static extern void GvrKeyboardSetEyeData(int eye_type, Matrix4x4 modelview, Matrix4x4 projection,
            gvr_recti viewport);
 ...
 packageManager.Call<AndroidJavaObject>(METHOD_NAME_GET_PACKAGE_INFO, PACKAGE_NAME_VRINPUTMETHOD, 0);
            if (info == null)
            {
